...
- We want to save it in an `.R` file in our `R/` subdirectory folder of our package (we saw this above already).
...
- Note: After executing the `use_r()` command we can use our function although it is **not defined in our global environment**. 
...
After running a lot of checks the `check`<sup>1</sup> function finally gives us a summary that tells us how many errors, warnings or notes are returned.
...
- A big advantage of packages and the functions that come along is the possibility to call the help `?` function.

- Let's add useful documentation around our package and our function `sq()` in order to make useful help. 

- For doing so, we need a special R documentation file: `man/sq.Rd`<sup>2</sup>.
...
- After adding relevant information, such as the type of input the function accepts or a minimal working example we need to update our package so that it includes our specified roxygen comments. We can do so using:
