...
After running a lot of checks we get a summary at the end that tells us how many errors, warnings or notes are returned.
...
Since software licensing is a rather complicated field because it lies at the intersection between programming and law, we will simplify this part. The license we give basically indicates how authors want their code to be treated.

In our example we will go for the `use_mit_license()`. But if you're interested in this topic you can read more here:
...
A big advantage of packages is the possibility to call the help `?` function. Let's add useful document around our package and our function `sq()`. For doing so, we need a special R documentation file, `man/sq.Rd` (why man??).
